...
The plots below use `ggplot2`, which was last week's session, so the code should be readable. Everything here is the same handful of pieces: `ggplot()` and `aes()` to say what goes where, a `geom_` to draw it, `coord_fixed()` or `coord_cartesian()` to fix the axes, `labs()` for the labels.
...
`geom_abline()` draws a straight line from an intercept and a slope; intercept 0 and slope 1 is the diagonal. `annotate("point", ...)` puts markers at given coordinates without needing a data frame.

`coord_fixed()` does two jobs. Its `xlim` and `ylim` arguments give the range each axis shows, which here forces the full 0-to-100 square rather than letting the plot shrink to fit the data. And it makes **one unit the same physical length on both axes** — a percentage point is as long going up as it is going across. That matters whenever the two axes carry the same scale. The dashed line means "equal" only if it runs at 45 degrees; let ggplot stretch the plot to fill the page and the same line of equality can lean steep or flat, and the eye reads a gap that is not there. When the two axes carry different scales there is no such line to protect, and `coord_cartesian()`, which sets the limits without fixing the ratio, is the right tool — it appears further down.
...
`geom_hline()` draws a horizontal line at a given height on the vertical axis. Dollars across and years up are not the same scale, so there is no line of equality to protect and `coord_cartesian()` is the right choice here: it sets the range shown — only `ylim` this time, so that the whole 0-to-100 span of the outcome is visible — and leaves the plot free to fill the page. `scale_x_continuous(labels = ...)` controls how the numbers on that axis are printed; `scales::label_comma()` writes them with thousands separators, and without it R writes them as `5e+04`, which is not something to put in front of a reader.
...
`annotate("polygon", ...)` fills the triangle defined by three corners. Both variables take only eleven whole-number values, so `r nrow(voters)` respondents sit on at most 121 positions, and the dots are drawn transparent so that a position holding many of them comes out darker. This is the one place not to jitter. The displacement is random in every direction, so dots on the diagonal --- people who rated their own party highest, which is most of them --- would be scattered into the shaded region, and the plot would contradict the sentence above it. Wherever a plot carries a hard limit, the display must respect it as strictly as the data do.
...
| Same scale on both axes: one unit the same length on each | `coord_fixed(xlim = , ylim = )` |
