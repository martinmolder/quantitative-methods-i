...
This session is about how to make a plot in R. Next week is about what belongs on one — which is the harder question, and the one that needs these tools to answer.
...
`geom_point()` draws one dot per case. That plot is nearly useless, though, and the reason is worth understanding: both variables take eleven whole-number values, so `r format(nrow(swe), big.mark = ",")` respondents are stacked on 121 positions. Every dot looks the same whether one person or four hundred are underneath it.

The way out is to stop the dots from landing on exactly the same spot:
...
`geom_jitter()` is `geom_point()` with a small random displacement added to every dot, so that the cases sharing a pair of values spread into a cloud instead of hiding behind one another. `width` and `height` say how far, at most, a dot may move in each direction, in the units of the axes --- a quarter of a point here, which keeps every cloud clearly around its own grid position. `alpha` makes the dots transparent, so that where many overlap the cloud is darker. Now the two dense corners are visible: people who like the Social Democrats dislike the Moderates, and the other way round.

The displacement is noise added for display and nothing else. The data have not changed, and a reader has to be told the points are jittered, in the caption or the text, or they will read the spread as real.
...
> **Next week** we come back to plots from the other direction: not how to draw one, but what has to be on it, and what you can work out about a relationship before you have looked at any data at all. Everything in this chapter is the equipment for that.

::: {.swirl}
**In the seminar.** `swirl` lesson **04 Visualisation**. The grammar, one-variable and two-variable plots, mapping against setting, faceting, and labelling a figure properly. Around 30 minutes.
:::
