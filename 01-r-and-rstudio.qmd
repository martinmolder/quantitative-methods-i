...
The second half of every seminar is a `swirl` lesson. `swirl` is a package that turns the R console into an interactive tutorial: it asks you to do something, you type the command, and it tells you whether it worked before moving on. You are doing real work in a real console the whole time, which is why it is worth more than watching me type.

> **Today is the exception.** You were asked to install R and RStudio before this session, and in the second half we set `swirl` up together and walk through the first lesson as a demonstration — so that everyone has seen how it behaves before doing one on their own. From session 2 the half hour is yours to work through the lesson at your own pace.
>
> If the installation did not go smoothly, today is the time to say so.
...
The course lessons live on the web, and `install_course_url()` fetches them for you. There is nothing to download by hand. Its one argument is the address, in quotation marks:
...
That address is long, so copy it rather than typing it. It is also on the front page of these materials, under the downloads.

R will show a progress bar while it fetches about 2 MB, and then say nothing. Silence is success — you will see the lessons listed when you start `swirl` below.

Run the same `install_course_url()` line again whenever a new version is posted. It overwrites the lessons, and your progress is stored separately, so you do not lose it.

> **After updating, start the lesson fresh rather than resuming it.** When `swirl` offers to continue a lesson you had in progress, it restores the data and the questions as they were when you began — that is what "continue" means — so an updated lesson only takes effect if you start it again. Choose the option to start something new, then pick the lesson from the list.

If your network blocks the download — this happens on some university and workplace connections — take the `.zip` from the front page of these materials, put it in your project folder, and use `install_course_zip()` instead. It does the same job from a file you already have, and its argument is the path to that file:
...
`swirl` then asks your name, offers you the course, and lists the lessons in it. **They are numbered by seminar** — lesson `04 Visualisation` goes with session 4 and chapter 4 — so pick the number of the week you are in.
...
::: {.swirl}
**In the seminar.** `swirl` lesson **01 Getting Started**. Working with the console, objects, vectors, and missing values. Around 30 minutes.

We install `swirl` and the course lessons together and go through this one as a demonstration. **Bring a laptop with R and RStudio already installed** — that part is not what the half hour is for.
:::
