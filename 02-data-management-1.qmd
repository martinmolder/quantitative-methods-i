...
- [`cses6_swe_raw.csv`](data/cses6_swe_raw.csv) --- the survey as it comes from the archive
...
#| eval: false
download.file(
  url = "https://martinmolder.github.io/quantitative-methods-i/data/cses6_swe_raw.csv",
  destfile = "data/cses6_swe_raw.csv",
  mode = "wb"
)
```
...
>
> ```r
> raw$polint <- raw$F3001
> raw$polint[raw$polint %in% c(7, 8, 9)] <- NA
> ```
>
...
## Appendix: what the variables are
...
book <- read.csv("data/codebook.csv", stringsAsFactors = FALSE)
names(book)[1] <- "variable"

chapters <- sort(list.files(pattern = "^[0-9]{2}-.*[.]qmd$"))

code_in <- function(file) {
  lines <- readLines(file, warn = FALSE)
  fence <- grepl("^```", lines)
  chunks <- lines[cumsum(fence) %% 2 == 1 & !fence]
  inline <- unlist(regmatches(lines, gregexpr("`r [^`]+`", lines)))
  paste(c(chunks, inline), collapse = "\n")
}

code <- vapply(chapters, code_in, character(1))
numbers <- as.integer(sub("-.*", "", chapters))

sessions <- vapply(book$variable, function(v) {
  hits <- grepl(paste0("\\b", v, "\\b(?!\\()"), code, perl = TRUE)
  paste(numbers[hits], collapse = ", ")
}, character(1))
names(sessions) <- book$variable

n_used <- sum(nzchar(sessions))
```
...
`r n_used` of the `r nrow(book)` variables are used somewhere in the course. A blank in the last column means the variable is in the data set but we never touch it, and those are there deliberately: they are the obvious place to look if you want a question of your own to work on.

The same table is downloadable as [`codebook.csv`](data/codebook.csv), which is easier to search than a web page.

```{r}
#| label: codebook-table
#| echo: false
#| results: asis

escape <- function(x) gsub("|", "\\|", x, fixed = TRUE)

for (group in unique(book$group)) {
  rows <- book[book$group == group, ]
  cat("### ", group, "\n\n", sep = "")
  cat("| Name | CSES name | What it is | Coding | Sessions |\n")
  cat("|---|---|---|---|---|\n")
  cat(sprintf(
    "| `%s` | `%s` | %s | %s | %s |\n",
    rows$variable, rows$source, escape(rows$label), escape(rows$coding),
    sessions[rows$variable]
  ), sep = "")
  cat("\n: {tbl-colwidths=\"[14,13,29,30,14]\"}\n\n")
}
```
